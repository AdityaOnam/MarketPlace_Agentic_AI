...
## D-036 — Phase 10: classifier receives the complete collected evidence contract; vocabulary cannot independently establish institutional/SaaS identity

**Date:** 2026-09-13
**Status:** Applied locally; pending user-authorised commit.

### Context

P10-13 found that the ignored local evaluation driver (`harness/collect.py`) reduced
each fetched page to six fields before calling the already-capable D-034 classifier.
That discarded nested JSON-LD, extracted links, title/meta text, static content and
robots/sitemap hints. The classifier itself supports the complete page shape. The
reduction made well-sampled SaaS sites such as Salesforce and Databricks `unknown`.

P10-14 found the inverse risk once the full page contract was used: vocabulary can
describe an article's topic rather than the site operator. On a full-evidence replay,
Paul Graham's essays could otherwise make the site `institutional` from university
language alone.

### Decision

- The collector now calls `classify_archetype_detailed` with full successful page records,
  inventory, robots text and sitemap declarations. It persists the bounded decision
  explanation as `site.archetype_reason`; the orchestrator surfaces it in the report
  preamble as diagnostic evidence.
- Vocabulary may still break a tie, but cannot be the only positive evidence that
  establishes `institutional` or `saas_marketing`. An independent identity, structure
  or affordance signal is required. This is a general evidence-family rule, not a
  site/domain exception.
- `unknown` remains preferred to a forced label. In the replay, Itch becomes an explicit
  marketplace-versus-SaaS ambiguity rather than a news label; no downstream check is
  activated from that ambiguity.

### Verification and limits

- Static compilation passed for the modified collector, bundle builder, classifier and
  report composer. `harness/package.py` passed its manifest, skill, drift and dependency
  gates after generated bytecode caches were removed.
- Frozen replay confirmed these directionally useful changes: Salesforce and Databricks
  become `saas_marketing`; Yle becomes `news_editorial`; Plausible and Fastmail become
  `saas_marketing`; Schwab stays safely `unknown`; Paul Graham does not become
  `institutional`.
- The P10-10 sampler changes selection. Some old offline snapshots do not contain the
  newly selected URLs, so all corpus rows cannot be compared as a complete 90-report
  classifier score. Those offline misses are coverage limitations, not passes or score
  gains. New snapshots or a pre-labelled blind corpus are required before asserting an
  aggregate classifier improvement.
- Known changed labels including NNGROUP, Cloudflare and Creative Commons were retained
  in the diff for human review; no claim of blind accuracy gain is made. Blocked-site
  handling, access_state, Phase 9 gates and classifier thresholds are unchanged.

---

## D-037 — P10-10: decoded `go` redirect utilities are excluded from static-page sampling
...
The P10-10 sampler already rejected URL-shaped values in `return_to`, `redirect_uri`
and `next`, but the planned redirect-utility set also included `go`. A URL such as
`?go=https%3A%2F%2Fexample.test%2F` could therefore still be selected as a content page.

### Decision

- Add `go` to the same query-key guard. It is rejected only when its decoded value is
  URL-shaped (for example, an absolute URL or `/account`); ordinary scalar parameters
  such as `?go=5` remain eligible.
- This is a selection-only correction: it makes no requests, changes no robots logic,
  and does not alter the sampler's deterministic family-diversity behavior.

### Verification

- Focused checks cover encoded URL-shaped `go` rejection, scalar `go` preservation,
  existing session and `next` boundary cases, MediaWiki namespace exclusion, and stable
  family-diverse selection.
- An offline Mercado Libre replay completed with zero network requests and contained no
  `go` utility URL among the 20 selected static pages.
